...
describe("3×3 arazi", () => {
  it("3×3 ızgarada 12 iç dikiş var (6 doğu-batı + 6 kuzey-güney)", () => {
    ...
    // Kenar sayısı formülü: rows*(cols-1) + cols*(rows-1)
    expect(internalSeamPairs(5, 5)).toHaveLength(5 * 4 + 5 * 4);
    expect(internalSeamPairs(1, 1)).toHaveLength(0);
  });

  it("dokuz chunk kurulur, index paylaşılır, pozisyonlar ayrıdır", () => {
  ...
  it("FIELD normalleri BÜTÜN iç dikişlerde tam 0, MESH normalleri kırık", () => {
    const t = new Terrain(P);
    ...
    expect(mesh.maxHeight).toBe(0); // yükseklikler her modda aynı — dert normalde
    t.disposeAll();
  });

  it("setNormalSource pozisyonları DEĞİŞTİRMEZ, normalleri değiştirir", () => {
    const t = new Terrain(P);
  ...
  it("mergeChunks: 38.025 vertex, 73.728 üçgen, Uint32 index, dünya ofsetli pozisyon", () => {
    const t = new Terrain(P);
    ...
    // Son chunk'ın ilk vertex'i dünyada (2*64, ?, 2*64).
    ...
    // Index'ler chunk başına kaydırılmış ve sınır dışına taşmıyor.
    let max = 0;
  ...
  it("birleşik geometry TEK bounding sphere taşır, chunk'lı yol dokuz tane", () => {
    const t = new Terrain(P);
    const merged = t.mergeChunks();
    const spheres = t.chunkGeometries.map((g) => g.boundingSphere!.radius);
    expect(spheres).toHaveLength(9);
    // Birleşik kürenin yarıçapı tek chunk'ınkinden belirgin biçimde büyük:
    // culling granülaritesinin kaybı tam olarak bu.
    expect(merged.boundingSphere!.radius).toBeGreaterThan(Math.max(...spheres) * 2);
    merged.dispose();
    t.disposeAll();
  });

  it("disposeAll TOPLU boşaltır — geriye chunk kalmaz", () => {
    const t = new Terrain(P);
  ...
  it("kodun hiçbir yerinde Math.random yok — determinizm sözleşmesi", async () => {
    ...
    for (const root of roots) {
      for (const f of readdirSync(root)) {
        ...
        // Kendi satırımız hariç: bu testin kaynağında geçen dizgeyi saymayalım.
